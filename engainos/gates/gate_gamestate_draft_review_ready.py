...
def gate_draft_contract_valid(draft: Dict[str, Any]) -> GateResult:
    ...
    """
    if draft.get("contract") != "engainos.gamestate_draft.v1":
        return GateResult(
            "GATE_DRAFT_CONTRACT_VALID",
            "FALSE",
            f"Invalid contract: {draft.get('contract')}",
            {"expected": "engainos.gamestate_draft.v1", "actual": draft.get("contract")}
        )
...
def gate_draft_source_valid(draft: Dict[str, Any]) -> GateResult:
    ...
    """
    if draft.get("source") != "engainos.zonj_runtime_acceptance_adapter":
        return GateResult(
            "GATE_DRAFT_SOURCE_VALID",
            "FALSE",
            f"Invalid source: {draft.get('source')}",
            {"expected": "engainos.zonj_runtime_acceptance_adapter", "actual": draft.get("source")}
        )
...
def build_draft_review_result(draft: Dict[str, Any]) -> DraftReviewResult:
    ...
    provisional_result = DraftReviewResult(
        contract="engainos.gamestate_draft_review_result.v1",
        source="engainos.gamestate_draft_review_gate",
        authority_tier=1,
