...
def run_narrative_pipeline(
    ...
    try:
        # Copy input to work_dir so pass1 output lands there too
        work_input = work_dir / input_txt.name
        if not work_input.exists():
            shutil.copy2(str(input_txt), str(work_input))

        # Pass 1 — takes ONE arg, writes out_pass1_* next to input
        r = subprocess.run(
        ...
        if not pass1_out.exists():
            alt = list(work_dir.glob(f"out_pass1*{stem}*"))
            if alt:
                pass1_out = alt[0]
            else:
                return False, f"Pass 1 produced no output in {work_dir}", meta

        # Pass 2 — takes ONE arg, writes .metta next to input
        r = subprocess.run(
            [sys.executable, str(pipeline_dir / "pass2_core.py"),
             str(pass1_out)],
            capture_output=True, text=True, timeout=120,
            cwd=str(work_dir),  # run from work_dir so output lands there
        )
        if r.returncode != 0:
            return False, f"Pass 2 failed: {r.stderr[:500] or r.stdout[:500]}", meta

        # Pass 2 writes output next to input — find it
        if not pass2_out.exists():
            alt = list(work_dir.glob(f"out_pass2*{stem}*")) + \
                  list(work_dir.glob(f"*{stem}*.metta"))
            if alt:
                pass2_out = alt[0]
            else:
                return False, f"Pass 2 produced no .metta output in {work_dir}", meta
