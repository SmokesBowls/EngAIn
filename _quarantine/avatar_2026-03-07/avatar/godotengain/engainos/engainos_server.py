...
# Lore Bridge Imports
from tools.lore_compiler.engain_semantic_compiler import BurdensSemanticCompiler
from tools.lore_compiler.zw_gatekeeper import json_to_zw
...
class LoreUpliftReq(BaseModel):
    text: str
    chapter_id: Optional[str] = "unknown"
...
@app.post("/api/lore/uplift")
def lore_uplift(req: LoreUpliftReq):
    """
    Uplift raw narrative text into executable ZW blocks and runtime events.
    """
    compiler = BurdensSemanticCompiler()
    result = compiler.compile_chapter(req.text)
    
    # Package for Godot and Internal Logs
    uplifted_package = {
        "chapter_id": req.chapter_id,
        "signals_count": len(result["events"]),
        "visual_ops": result["visual_ops"],
        "logic_deltas": result["module_updates"],
        "zw_blocks": []
    }
    
    # Convert semantic events to ZW blocks via Gatekeeper
    for event_json in result["events"]:
        raw_event = json.loads(event_json)
        try:
            # Normalized for ZW
            zw = json_to_zw({
                "id": f"lore_{req.chapter_id}_{raw_event['data'].get('type', 'event')}",
                "type": "narrative_sync",
                "scope": "LoreBridge",
                "action": raw_event['data'].get('type', 'sync'),
                "context": raw_event['data']
            })
            uplifted_package["zw_blocks"].append(zw.to_runtime_packet())
        except Exception as e:
            print(f"Lore Uplift warning: {e}")

    # Optionally push deltas to the running simulation
    if req.apply_to_runtime and result["module_updates"].get("delta"):
        for key, value in result["module_updates"]["delta"].items():
            rt.command({"action": "update_world_state", "property": key, "delta": value})

    return uplifted_package
...
# ----------------------------
# Mr. Lore & Dragon Collaboration
# ----------------------------

@app.post("/api/lore/collaborate")
async def lore_collaborate(
    intent: str = Form(...),
    context: str = Form(""),
):
    """
    Mr. Lore's Collaboration Brain.
    Interprets natural language to drive Blender/Trae agents.
    """
    print(f"[MR LORE] Processing intent: {intent}")
    
    response_text = ""
    actions = []
    
    intent_lower = intent.lower()
    if "tower" in intent_lower or "obelisk" in intent_lower:
        response_text = "The cosmology demands a Phase-Shift Obelisk. I am signaling the Blender MCP and Trae Agent."
        actions.append({
            "agent": "blender",
            "op": "generate_mesh",
            "params": {"prompt": "sci-fi obsidian tower", "id": "obelisk_01"}
        })
        actions.append({
            "agent": "trae",
            "op": "place_node",
            "params": {"node_name": "PhaseObelisk", "mesh_id": "obelisk_01", "pos": [0, 10, 0]}
        })
    elif "light" in intent_lower or "sun" in intent_lower:
        response_text = "Adjusting the cosmic luminosity anchors."
        actions.append({
            "agent": "trae",
            "op": "modify_environment",
            "params": {"light_intensity": 2.5, "color": "sky_blue"}
        })
    else:
        response_text = f"I acknowledge your intent: '{intent}'. I am consulting the ZW Chronicles for an appropriate manifestation."
        
    return {
        "mr_lore_response": response_text,
        "actions": actions,
        "status": "COLLABORATION_TRIGGERED"
    }


@app.post("/api/dragon/speak")
async def dragon_speak(
    text: str = Form(...),
):
    """
    Processes dragon communication and returns an AI-flavored response.
    """
    # This acts as a pass-through to an LLM or a simple rule-based draconic filter
    draconic_response = f"EngAIn Dragon pulses with resonance: '{text}' ... Calculating cosmic delta."
    return {"response": draconic_response}
