...
class ZWEditorGUI:
    ...
    def _bind_shortcuts(self):
        ...
        # Bind events to check changes and update cursor position
        self.zw_editor.bind('<KeyRelease>', self.on_editor_event)
        self.zw_editor.bind('<ButtonRelease>', self.on_editor_event)
        self.zw_editor.bind('<<Modified>>', self.on_editor_event)
    ...
    def on_editor_event(self, event=None):
        """Handle editor events: check for changes and update cursor position"""
        self.check_changes()
        self.update_cursor_position()

        # Reset the modified flag so future <<Modified>> events fire
        if event and event.type == tk.EventType.VirtualEvent and event.widget.edit_modified():
            event.widget.edit_modified(False)

    def update_cursor_position(self):
        """Update the cursor position label"""
        cursor_index = self.zw_editor.index(tk.INSERT)
