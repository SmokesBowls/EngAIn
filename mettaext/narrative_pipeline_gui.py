    ...
    def process_narrative(self):
        ...
        try:
            ...
            if result.returncode != 0:
                raise Exception(f"Pass 1 failed: {result.stderr}")
            ...
            if result.returncode != 0:
                raise Exception(f"Pass 2 failed: {result.stderr}")
            ...
            if result.returncode != 0:
                raise Exception(f"Pass 3 failed: {result.stderr}")
...
Output Directory: {zon_output_dir}"""
    ...
    def convert_lore(self):
        ...
        try:
            ...
            if result.returncode != 0:
                raise Exception(f"Lore conversion failed: {result.stderr}")
