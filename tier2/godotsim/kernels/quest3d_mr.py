...
def step_quest3d(
    ...
    for delta in deltas:
        ...
        elif delta_type == "quest/complete":
            # AP-emitted: directly complete a quest (bypasses objective check)
            quest_id = delta.get("quest_id", "")
            quest = state.quests.get(quest_id)
            if quest and quest.status == "active":
                completed = Quest(
                ...
                accepted_ids.append(delta_id)
                alerts.append({"type": "quest_completed", "quest_id": quest_id, "source": "ap_rule"})

        elif delta_type == "quest/fail":
            # AP-emitted: directly fail a quest
            quest_id = delta.get("quest_id", "")
            quest = state.quests.get(quest_id)
            if quest and quest.status == "active":
                failed = Quest(
                ...
                state = state.with_quest(quest_id, failed)
                accepted_ids.append(delta_id)
                alerts.append({"type": "quest_failed", "quest_id": quest_id, "source": "ap_rule"})
...
def get_quest_summaries(snapshot: Dict) -> List[Dict]:
    """
    Generate flat QuestSummary list from world snapshot.
    This is ALL the QuestTracker UI needs. Pure read-only projection.
    ...
    """
    quests = snapshot.get("quest", {}).get("quests", {})
    summaries = []
    for qid, quest in quests.items():
        if isinstance(quest, dict):
            objectives = quest.get("objectives", [])
        else:
            objectives = [{"id": o.id, "description": o.description, "status": o.status, "optional": o.optional} for o in quest.objectives]
            quest = {"id": quest.id, "title": quest.title, "status": quest.status, "description": quest.description, "objectives": objectives}
        ...
        summaries.append({
            ...
            "objectives": [
                {"id": o.get("id", ""), "description": o.get("description", ""), "status": o.get("status", "pending"), "optional": o.get("optional", False)}
                for o in objectives
            ]
        })
    return summaries
...
if __name__ == "__main__":
    ...
    # ── Test 9: AP-emitted quest/complete delta ──
    print("\n[Test 9] AP delta: quest/complete")
    snapshot9 = {
        "entities": {"player": {"flags": {}, "items": {}, "location": "village"}},
        "quest": {"quests": {
            "q_ap_test": {"id": "q_ap_test", "title": "AP Test", "status": "active",
                "objectives": [{"id": "o1", "description": "Test", "status": "pending", "conditions": []}],
                "rewards": [{"type": "set_flag", "target_entity": "player", "key": "ap_done", "value": True}],
                "preconditions": [], "failure_conditions": [],
                "activated_at_tick": 0.0, "completed_at_tick": -1.0, "failed_at_tick": -1.0}
        }, "tick": 1.0}
    }
    deltas9 = [{"type": "quest/complete", "id": "ap_complete_1", "quest_id": "q_ap_test"}]
    out9, acc9, alerts9 = step_quest3d(snapshot9, deltas9, config, 0.016)
    assert "ap_complete_1" in acc9
    assert out9["quest"]["quests"]["q_ap_test"]["status"] == "completed"
    assert any(a["type"] == "quest_completed" and a.get("source") == "ap_rule" for a in alerts9)
    assert out9["entities"]["player"]["flags"].get("ap_done") == True
    print("  ✅ AP quest/complete accepted, rewards applied")
    passed += 1

    # ── Test 10: AP-emitted quest/fail delta ──
    print("\n[Test 10] AP delta: quest/fail")
    snapshot10 = copy.deepcopy(snapshot9)
    snapshot10["quest"]["quests"]["q_ap_test"]["status"] = "active"
    deltas10 = [{"type": "quest/fail", "id": "ap_fail_1", "quest_id": "q_ap_test"}]
    out10, acc10, alerts10 = step_quest3d(snapshot10, deltas10, config, 0.016)
    assert "ap_fail_1" in acc10
    assert out10["quest"]["quests"]["q_ap_test"]["status"] == "failed"
    assert any(a["type"] == "quest_failed" and a.get("source") == "ap_rule" for a in alerts10)
    print("  ✅ AP quest/fail accepted")
    passed += 1

    # ── Test 11: get_quest_summaries ──
    print("\n[Test 11] QuestSummary projection")
    summary_snap = {
        "entities": {},
        "quest": {"quests": {
            "q1": {"id": "q1", "title": "Active Quest", "status": "active", "description": "Testing",
                   "objectives": [
                       {"id": "o1", "description": "Step 1", "status": "satisfied", "optional": False},
                       {"id": "o2", "description": "Step 2", "status": "pending", "optional": False},
                       {"id": "o3", "description": "Bonus", "status": "pending", "optional": True},
                   ]},
            "q2": {"id": "q2", "title": "Done Quest", "status": "completed", "description": "",
                   "objectives": [{"id": "o1", "description": "Done", "status": "satisfied", "optional": False}]}
        }, "tick": 5.0}
    }
    summaries = get_quest_summaries(summary_snap)
    assert len(summaries) == 2
    q1_sum = next(s for s in summaries if s["id"] == "q1")
    assert q1_sum["progress"] == 1.0 / 3.0
    assert q1_sum["objectives_done"] == 1
    assert q1_sum["objectives_total"] == 3
    q2_sum = next(s for s in summaries if s["id"] == "q2")
    assert q2_sum["progress"] == 1.0
    print("  ✅ Summaries correct (progress, counts, flat structure)")
    passed += 1
