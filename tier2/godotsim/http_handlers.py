...
try:
    ...
    from tier3.mettaext.scene_identity import canonical_scene_id
except Exception:
...
class RuntimeHTTPHandler(BaseHTTPRequestHandler):
    ...
    def _handle_scene_load(self, body: Dict[str, Any]):
        ...
        def _first_scene_id_candidate(*values):
            for value in values:
                if isinstance(value, str) and value.strip():
                    return value.strip()
            return None

        embedded_scene = body.get("scene") if isinstance(body.get("scene"), dict) else None
        embedded_zonj = body.get("zonj") if isinstance(body.get("zonj"), dict) else None
        requested_id_raw = _first_scene_id_candidate(
            body.get("scene_id"),
            body.get("@id"),
            body.get("id"),
            embedded_scene.get("scene_id") if embedded_scene else None,
            embedded_zonj.get("scene_id") if embedded_zonj else None,
            embedded_scene.get("@id") if embedded_scene else None,
            embedded_zonj.get("@id") if embedded_zonj else None,
        )
        requested_id = canonical_scene_id(requested_id_raw) if requested_id_raw else None
        body_entities = body.get("entities") or (embedded_scene.get("entities") if embedded_scene else None) or (embedded_zonj.get("entities") if embedded_zonj else None) or []
        body_segments = body.get("segments") or body.get("events") or (embedded_scene.get("segments") if embedded_scene else None) or (embedded_zonj.get("segments") if embedded_zonj else None) or []
        body_entities_count = len(body_entities) if isinstance(body_entities, (list, dict)) else 0
        body_segments_count = len(body_segments) if isinstance(body_segments, (list, dict)) else 0
        print(
            "[TRACE_SCENE030][HTTP_SCENE_LOAD_ENTRY] "
            f"requested_id_raw={requested_id_raw} requested_id={requested_id} "
            f"body.scene_id={body.get('scene_id')} body.@id={body.get('@id')} body.id={body.get('id')} "
            f"source_path={body.get('source_path') or ((body.get('file') or {}).get('source_path') if isinstance(body.get('file'), dict) else None)} "
            f"body_entities={body_entities_count} body_segments={body_segments_count}"
        )

        doc = embedded_zonj or embedded_scene
        vault_scenes = getattr(self.runtime, "vault_scenes", {})
        used_registered_vault_scene = False
        if requested_id and isinstance(vault_scenes, dict) and requested_id in vault_scenes:
            # Requested registered vault scene owns identity. Do not let an embedded
            # malformed mini-doc (e.g. id "?", "_", "scene._") replace it.
            doc = vault_scenes[requested_id]
            used_registered_vault_scene = True
        elif not doc:
            doc = {k: v for k, v in body.items() if k not in ("command", "action")}

        doc = normalize_scene_doc(doc)
        doc_id_raw = doc.get("@id") or doc.get("scene_id") or (doc.get("id") if doc.get("type") == "scene" else None) or "unknown"
        target_scene_id = requested_id or canonical_scene_id(doc_id_raw)
        doc["@id"] = target_scene_id
        doc["scene_id"] = target_scene_id
        doc_entities = doc.get("entities") or doc.get("@entities") or []
        doc_segments = doc.get("segments") or doc.get("=segments") or []
        print(
            "[TRACE_SCENE030][HTTP_DOC_RESOLVED] "
            f"target_scene_id={target_scene_id} requested_id={requested_id} "
            f"used_registered_vault_scene={used_registered_vault_scene} "
            f"doc.@id={doc.get('@id')} doc.scene_id={doc.get('scene_id')} "
            f"doc_entities={len(doc_entities) if isinstance(doc_entities, (list, dict)) else 0} "
            f"doc_segments={len(doc_segments) if isinstance(doc_segments, (list, dict)) else 0}"
        )

        print(
            "[SCENE_LOAD_ID_AUDIT] "
            f"requested_id_raw={requested_id_raw} requested_id={requested_id} "
            f"doc_id_raw={doc_id_raw} target_scene_id={target_scene_id} "
            f"used_registered_vault_scene={used_registered_vault_scene}"
        )
        ...
        print(f"[TRACE_SCENE030][HTTP_BEFORE_SCENE_MANAGER] target_scene_id={target_scene_id} runtime_active_before={getattr(self.runtime, '_active_scene_id', None)} snapshot_scene_before={(getattr(self.runtime, 'snapshot', {}) or {}).get('scene_id') if isinstance(getattr(self.runtime, 'snapshot', {}), dict) else None}")
        print(f"[SCENE_MANAGER_ID_AUDIT] load_scene target_scene_id={target_scene_id} override_id={target_scene_id}")
        self.runtime.scene_manager.load_scene(doc, activate=True, override_id=target_scene_id)
        scene_id = target_scene_id
        snapshot_after = getattr(self.runtime, "snapshot", {}) if isinstance(getattr(self.runtime, "snapshot", {}), dict) else {}
        bridge_after = snapshot_after.get("bridge_entities") or []
        print(f"[TRACE_SCENE030][HTTP_AFTER_SCENE_MANAGER] scene_id={scene_id} snapshot_scene_after={snapshot_after.get('scene_id')} bridge_entities_scene_id={snapshot_after.get('bridge_entities_scene_id')} bridge_entities={len(bridge_after) if isinstance(bridge_after, (list, dict)) else 0}")
        ...
        try:
            ...
            self.runtime._active_scene_id = target_scene_id
            self.runtime._active_scene_doc = doc
            print(f"[SCENE_SELECT_ID_AUDIT] active_scene_id={self.runtime._active_scene_id} doc_scene_id={doc.get('scene_id')} doc_at_id={doc.get('@id')}")
        ...
        print(f"[TRACE_SCENE030][HTTP_RESPONSE] scene_id={scene_id} status=loaded runtime_active={getattr(self.runtime, '_active_scene_id', None)} snapshot_scene={(getattr(self.runtime, 'snapshot', {}) or {}).get('scene_id') if isinstance(getattr(self.runtime, 'snapshot', {}), dict) else None}")
