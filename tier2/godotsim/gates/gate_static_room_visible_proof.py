#!/usr/bin/env python3
"""
gate_static_room_visible_proof.py
Proves a static room (floor, wall, camera, light) builds and renders visibly in Godot.
No auto-exit script. Camera is current and directional light illuminates the scene.
"""
...
SCENE_PATH = ROOT / "tmp_static_room_gate_scene.tscn"
...
def main():
    print("=" * 52)
    print("RUNNING GATE: gate_static_room_visible_proof.py")
    print("=" * 52)

    # 1. Define demand pieces list (floor, wall, camera, light)
    static_room_demand = {
        "scene_id": "static_room_gate_scene",
        "pieces": [
            {
                "piece_id": "floor_main",
                "piece_type": "floor",
    ...
    print("[gate_static_room_visible_proof] 1. Validating static room pieces via builder...")
    status, reasons = build_godot_scene(static_room_demand, SCENE_PATH, MANIFEST_PATH)
    print(f"Build Result: {status} - Reasons: {reasons}")
    if status != STATUS_BUILT:
        print("FAIL: Static room scene could not be built.")
        sys.exit(1)

    # 2. Launch Godot visibly if available
    print("[gate_static_room_visible_proof] 2. Launching Godot with display, holding window open 4s...")
    godot_bin = find_godot_binary()
    if not godot_bin:
        print("BYPASS: Godot binary not found in environment.")
        print("====================================================")
        print("gate_static_room_visible_proof: BYPASS")
        print("====================================================")
        sys.exit(0)

    print(f"Found Godot binary at: {godot_bin}")
    print("*** A WINDOW SHOULD APPEAR SHOWING A FLOOR, A WALL, AND A DIRECTIONAL LIGHT. ***")
    ...
    print("=" * 52)
    print("gate_static_room_visible_proof: TRUE")
    print("=" * 52)
