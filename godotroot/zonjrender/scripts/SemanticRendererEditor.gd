...
func _add_tile_to_surface(st: SurfaceTool, tx: int, tz: int, terrain: String, role: String) -> void:
	...
	var u0 := region.position.x / tw
	var v0 := region.position.y / th
	...
	st.set_normal(n)
	st.add_vertex(Vector3(x0, TERRAIN_Y, z0))
	st.set_uv(Vector2(u1, v0))
	st.set_normal(n)
	st.add_vertex(Vector3(x1, TERRAIN_Y, z0))
	st.set_uv(Vector2(u1, v1))
	st.set_normal(n)
	st.add_vertex(Vector3(x1, TERRAIN_Y, z1))
	st.set_uv(Vector2(u0, v0))
	st.set_normal(n)
	st.add_vertex(Vector3(x0, TERRAIN_Y, z0))
	st.set_uv(Vector2(u1, v1))
	...
	st.set_normal(n)
	st.add_vertex(Vector3(x0, TERRAIN_Y, z1))
...
func _terrain_at(x: int, z: int) -> String:
	if z < 0 or z >= _terrain_grid.size():
		return ""
