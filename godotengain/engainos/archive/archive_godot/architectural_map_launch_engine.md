...
│   ├── mesh_intake.py       [ ] Exists?
│   ├── mesh_manifest.py     [ ] Exists?
│   ├── scene_server.py      [ ] Has `start_scene_server`?
│   ├── godot_adapter.py     [ ] Imports cleanly?
│   ├── ap_engine.py         [ ] Has `ZWAPEngine`, `StateProvider`?
│   └── ap_runtime.py        [ ] Has `APRuntimeIntegration`?
├── tools/
│   └── trixel/              [ ] Directory exists?
├── godot/                   [ ] Directory exists?
├── assets/
│   └── trixels/             [ ] Auto-created by launcher
└── game_scenes/             [ ] Used by AP Runtime
...
python
# Line 53: Unclosed quote
raise FileNotFoundError(f "Mesh not found: {obj_path} ")
# Should be:
raise FileNotFoundError(f"Mesh not found: {obj_path}")

# Line 60: Space in string
zw_concept= zw_concept,  # Extra space after =
# Line 66: Space in string
export_format= "obj ",  # Trailing space
# Line 71: Space in variable
manifest.anchors = an chors  # Typo: 'an chors'
# Line 75: Space in string
f "INTAKE REJECTED: {obj_path}\n " +
2. scene_server.py - Missing Import

python
# Line 9: This module does not exist
from scene_loader import SceneLoader, format_for_godot
# ACTION: Need to create core/scene_loader.py
3. godot_adapter.py - Missing Imports

python
# Line 17-18: These modules do not exist
from zon_to_entities import zon_scene_to_entities
from spatial_skin_system import build_render_plan
# ACTION: Need to create both modules

4. mesh_manifest.py - Syntax Errors

python
# Multiple spaces in strings throughout:
"torso ",  "shield "  # Trailing spaces
"solid ",  "trigger "  # Trailing spaces
"1.0 "  # Trailing space
# Line 225: Typo in variable
s ource_dir = ...  # Space in variable name
# Line 238: Typo in variable
manifes t.validate()  # Space in variable name
...
python
class ZWAPEngine:
    def list_rules(self) -> List[Dict]:  # Returns [{'id': str, 'priority': int}, ...]
        ...
...
class APRuntimeIntegration:
    ...
    def initialize(self, initial_state: dict): ...  # Keys: flags, stats, locations, inventory
    def handle_message(self, msg_dict: dict) -> dict: ...
    @property
    def engine(self) -> ZWAPEngine: ...
...
def format_for_godot(scene: dict) -> dict: ...
zon_to_entities.py

python
def zon_scene_to_entities(scene_ dict, auto_bind_skins: bool = False) -> List: ...
spatial_skin_system.py

python
def build_render_plan(entity) -> RenderPlan: ...
...
python
1def _handle_simulate_tick(self, msg: Dict) -> Dict:
    # ...
    context = msg.get('context', {})
    # ❌ MISSING: result = self.engine.simulate_tick(context)
    return {
        'type': 'ap_simulate_result',
        'result': result  # 💥 NameError: name 'result' is not defined
    }
...
1def _handle_simulate_tick(self, msg: Dict) -> Dict:
    ...
    return {
        'type': 'ap_simulate_result',
        'result': result  # 💥 NameError when Godot sends 'ap_simulate_tick'
    }
