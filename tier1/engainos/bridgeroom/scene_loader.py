...
try:
    from tier3.mettaext.scene_identity import to_canonical_scene_id
...
class SceneLoader:
    ...
    def load_scene(self, scene_id: str) -> Dict[str, Any]:
        ...
        for stem in unique_candidates:
            ...
            if path.exists():
                ...
                # Normalize chapter/stage language for runtime compatibility.
                #
                # chapter_id = authored/compiled chapter packet identity
                # stage_id   = runtime/display scene identity inside the chapter
                # scene_id   = compatibility field expected by old Godot/runtime code
                #
                # If a packet has stage_id, that is the real runtime scene_id.
                # If it only has chapter_id, fall back to chapter_id.
                # If it only has old scene_id, preserve it.
                if "chapter_id" not in packet:
                    packet["chapter_id"] = packet.get("scene_id", canonical)

                if "stage_id" in packet:
                    packet["scene_id"] = packet["stage_id"]
                elif "scene_id" not in packet:
                    packet["scene_id"] = packet.get("chapter_id", canonical)

                packet["source_packet_id"] = canonical
                return packet
    ...
    def list_available_scenes(self) -> List[str]:
        """
        List loadable compiled packet IDs.
        ...
        return [
            p.stem
            for p in self.scenes_dir.glob("*.json")
