...
class SceneRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            if parsed.path in ('/load_scene', '/load_chapter'):
                scene_id = params.get('scene_id', [None])[0] or params.get('chapter_id', [None])[0]
                
                if not scene_id:
                    status_code = 400
                    response = {'status': 'error', 'message': "Missing scene_id/chapter_id parameter"}
                else:
                    try:
                        scene = self.loader.load_scene(scene_id)

                        if parsed.path == '/load_chapter':
                            response = {
                                'status': 'success',
                                'packet_type': 'compiled_chapter',
                                'chapter_id': scene_id,
                                'legacy_scene_id': scene.get('scene_id'),
                                'data': scene,
                            }
                            print(f"[SceneServer] Served compiled chapter: {scene_id}")
                        else:
                            response = {'status': 'success', 'data': scene}
                            print(f"[SceneServer] Served scene compatibility route: {scene_id}")
            ...
            # Route: /list_scenes
            elif parsed.path in ('/list_scenes', '/list_chapters'):
                scenes = self.loader.list_available_scenes()
                response = {'status': 'success', 'chapters': scenes, 'scenes': scenes}
                status_code = 200
