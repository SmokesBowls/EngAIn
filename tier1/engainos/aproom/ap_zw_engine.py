...
MIGRATED_AP_ZW_ENGINE_MODULE = "tier1.engainos.core.ap_zw_engine"
...
class ZWAPEngine:
    ...
    def __init__(
        ...
        # Timeline writing is disabled by default.
        # This keeps migrated ZW AP execution quarantined until EngAInOS explicitly allows writes.
    ...
    def _append_zon_event(self, entry: Dict):
        ...
        Timeline writing is fenced.
        By default, migrated ZW AP execution does not write to disk.
        A caller must explicitly construct ZWAPEngine(enable_timeline_write=True).
        """

        if not self.enable_timeline_write:
            self._log_warning("timeline_write_skipped: enable_timeline_write is False")
            return

        # Ensure zon directory exists relative to explicit timeline_root, ENGAIN_ROOT, or repo root.
        if self.timeline_root:
            root_dir = Path(self.timeline_root).resolve()
        else:
