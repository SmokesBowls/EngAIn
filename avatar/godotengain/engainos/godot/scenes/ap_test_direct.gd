...
const RUNTIME_URL = "http://127.0.0.1:8765/ap_query"
...
func _ready():
    # Attempt to find the cube either as parent or sibling
    var parent = get_node_or_null("..")
    if parent is MeshInstance3D:
        cube = parent
    else:
        # Fallback to sibling named MeshInstance3D
        cube = get_node_or_null("../MeshInstance3D")

    if cube == null:
        print("[AP Test] ERROR: MeshInstance3D not found as parent or sibling!")
        return

    print("[AP Test] Found cube: ", cube.get_path())
