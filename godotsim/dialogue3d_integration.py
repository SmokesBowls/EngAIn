...
class Dialogue3DAdapter:
    ...
    def get_all_state(self) -> Dict[str, Any]:
        """Get full dialogue state (JSON-safe)"""
        # Convert sets to lists for JSON serialization
        safe_state = {
            "entities": {},
            "conversations": self.state.get("conversations", {})
        }
        for eid, edata in self.state.get("entities", {}).items():
            safe_state["entities"][eid] = {
                "reputation": edata.get("reputation", {}),
                "knowledge_flags": list(edata.get("knowledge_flags", set())),
                "responses_given": list(edata.get("responses_given", set())),
