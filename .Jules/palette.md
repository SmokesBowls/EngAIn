...
## 2024-06-15 - Add Select All Shortcut to Tkinter Text Widgets
**Learning:** Tkinter's `Text` and `ScrolledText` widgets lack consistent cross-platform support for the 'Select All' (`Ctrl+A`) shortcut. Without an explicit binding, users cannot easily select all text in input or output panes.
**Action:** Explicitly bind both `<Control-a>` and `<Control-A>` to a custom method that applies the `tk.SEL` tag from `1.0` to `tk.END` and returns `'break'` to ensure cross-platform 'Select All' functionality and prevent duplicate event handling. Pass the widget context via lambda to ensure the correct text area is selected.
