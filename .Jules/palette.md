...
## 2024-10-25 - Fix inconsistent "Select All" in Tkinter text widgets
**Learning:** Tkinter's `Text` and `ScrolledText` widgets lack consistent cross-platform support for the "Select All" keyboard shortcut (Ctrl+A). Without explicit bindings, users on some platforms cannot easily select all text, which is a fundamental text editing interaction.
