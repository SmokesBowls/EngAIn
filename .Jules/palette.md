...
**Learning:** Tkinter's `Text` and `ScrolledText` widgets lack consistent, built-in cross-platform support for the standard 'Select All' (Ctrl+A / Cmd+A) keyboard shortcut. Not having this makes text editing feel broken to users who expect native behavior.
**Action:** Always explicitly bind both `<Control-a>` and `<Control-A>` to a custom method that applies the `tk.SEL` tag from `1.0` to `tk.END` and crucially returns `'break'` to prevent duplicate event handling by the OS or default bindings.
