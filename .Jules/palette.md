...
## 2024-04-03 - [Fix Button Flash]
**Learning:** When styling Tkinter buttons with custom background colors for dark themes, clicking them causes a jarring visual flash to default system colors (often light gray) because the `activebackground` and `activeforeground` properties are not set.
**Action:** Always specify `activebackground` and `activeforeground` properties to match the theme and prevent the button from flashing default system colors during user interaction.
