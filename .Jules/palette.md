...
## 2024-06-17 - Add Select All shortcut to Tkinter Text Widgets
**Learning:** Tkinter's `Text` and `ScrolledText` widgets lack consistent cross-platform support for the 'Select All' shortcut. Explicitly binding both `<Control-a>` and `<Control-A>` to a custom method that applies the `tk.SEL` tag from `1.0` to `tk.END` and returns `'break'` ensures expected functionality and prevents duplicate event handling.
**Action:** When implementing text editors or output panels in Tkinter, always explicitly implement a "Select All" shortcut to ensure accessibility and consistent behavior across operating systems.
