...
## 2024-06-25 - Implement Reliable Select All for Tkinter Text Widgets
**Learning:** Tkinter's `Text` and `ScrolledText` widgets lack consistent cross-platform support for the 'Select All' shortcut (`Ctrl+A`). Relying on default bindings often leads to broken or duplicated events. Additionally, in headless testing environments (`xvfb-run`), `focus_get()` or `event.widget` inside generic event handlers fails reliably.
**Action:** Always explicitly bind both `<Control-a>` and `<Control-A>` to a custom method that applies the `tk.SEL` tag from `1.0` to `tk.END` and returns `'break'`. When binding to multiple widgets, explicitly capture the target widget in the lambda (e.g., `lambda e, w=widget: handler(e, w)`) to ensure reliable testability.
